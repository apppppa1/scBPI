...
# class Reconstruct_data(torch.nn.Module):
#     def __init__(self, output_dim: int, input_dim: int = 50, dropout = 0.2):
#         """
#         初始化Reconstruct_data类。
#
#         参数:
#         - input_dim: 输入数据的特征维度
#         - output_dim: 输出数据的目标特征维度
#         """
#         super(Reconstruct_data, self).__init__()
#
#         # 定义单层网络，直接从input_dim映射到output_dim
#         self.linear = torch.nn.Linear(input_dim, output_dim)
#         self.dropout = torch.nn.Dropout(dropout)  # 设置Dropout层，丢弃率为0.2
#     def forward(self, x):
#         """
#         执行前向传播，输入x，返回维度转换后的输出。
#
#         参数:
#         - x: 输入数据矩阵，形状为 (batch_size, input_dim)
#
#         返回:
#         - 维度为 (batch_size, output_dim) 的转换后数据矩阵
#         """
#         x_hat = self.linear(x)
#         x_hat = self.dropout(x_hat)  # 应用Dropout
#         return x_hat
    ...
    **kwargs
    ...
    def __init__(
        ...
        for k in self.net.keys:
            self.required_losses += [f"x_only_{k}_nll", f"x_only_{k}_kld", f"x_only_{k}_elbo", f"x_joint_{k}_nll", f"x_joint_{k}_kld", f"x_joint_{k}_elbo"]
        self.required_losses += ["elbo_sum", "vae_loss", "gen_loss","dsc_loss"]
        # self.required_losses += ["elbo_sum", "cluster_loss", "vae_loss", "gen_loss"]
