...
class NBDataDecoder(DataDecoder):
    ...
    def forward(
        ...
        scale = F.softplus(self.scale_lin[b])
        logit_mu = scale * x_hat + self.bias[b]
...
class Reconstruct_data(torch.nn.Module):
    def __init__(self, output_dim: int, input_dim: int = 100, dropout = 0.2):
        """
        初始化Reconstruct_data类。
